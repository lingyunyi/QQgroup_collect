...
def download(request):
    ...
    try:
        ...
        try:
            ...
            logger.exception("function users_disk_manager - %s - requestx.COOKIES.get is false - %s" % (ip, e),
                             exc_info=True)
            return redirect("/admins/login/")
        # -----------------执行验证------------------
        # -----------------执行验证------------------
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if admins_account != None and admins_passwd != None:
                logger.warning(
                    "function login - %s - post account not is null - %s - %s" % (ip, admins_account, admins_passwd))
                # 如果账号不为空
                ...
                if search_result_account != ():
                    ...
                    if search_result_account[0][1] == admins_account and search_result_account[0][2] == hashlib.md5(
                            admins_passwd.encode(encoding='UTF-8')).hexdigest():
                        # 303 账号或秘密错误,404密码不存在
                        logger.warning(
                            "function login - %s - login success - %s - %s" % (ip, admins_account, admins_passwd))
                        ...
                        if search_result_session == ():
                            ...
                            logger.warning("function login - %s - login success init start - %s - %s" % (
                                ip, admins_account, admins_passwd))
                            ...
                            if insert_result_session == True:
                                # 进行首页跳转，200，数据执行成功
                                logger.warning("function login - %s - insert_result_session susscess - %s - %s" % (
                                    ip, admins_account, admins_passwd))
                                ...
                                logger.warning("function login - %s - updata_result_session susscess - %s - %s" % (
                                    ip, uuid4_str, admins_account))
                                return obj
                            if insert_result_session == False:
                                logger.warning("function login - %s - insert_result_session false - %s - %s" % (
                                    ip, admins_account, admins_passwd))
                                # 如果失败，返回重试，505请重试
                            ...
                            if updata_result_session == False:
                                logger.warning("function login - %s - updata_result_session false - %s - %s" % (
                                    ip, admins_account, admins_passwd))
                                # 如果失败，返回重试，505请重试
                                return HttpResponse("505")
                    else:
                        # 来到这里代表账号密码不匹配
                        logger.warning(
                            "function login - %s - login false - %s - %s" % (ip, admins_account, admins_passwd))
                        # 303 账号或秘密错误,404密码不存在
                        return HttpResponse("303")
                elif search_result_account == ():
                    # 账号不存在
                    logger.warning(
                        "function login - %s - login account is null - %s - %s" % (ip, admins_account, admins_passwd))
                    # 303 账号或秘密错误,404密码不存在
                    return HttpResponse("303")
                ...
                logger.warning(
                    "function login - %s - post account is null - %s - %s" % (ip, admins_account, admins_passwd))
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if search_admins_account_result != ():
                ...
                logger.warning("function users_manager - %s - search_admins_account_result = %s" % (
                    ip, search_admins_account_result))
                ...
                if search_admins_session_result != ():
                    ...
                    logger.warning("function users_manager - %s - search_admins_Session_result = %s" % (
                        ip, search_admins_session_result))
                    # 返回正常页面
                    if int(eval(search_admins_session_result[0][2]).get("alive_time_s")) <= int(time.time()):
                        # 代表管理员账户已过期
                        logger.warning(
                            "function users_manager - %s - search_admins_Session_result[0][3]['alive_time_s'] is false" % (
                                ip))
                        return redirect("/admins/login/")
                        ...
                        # ----------------------GET请求可以返回内容的开始---------------------------
                        logger.warning(
                            "function users_manager - %s - search_admins_Session_result[0][3]['alive_time_s'] is true" % (
                                ip))
                        ...
                        # 这里需要提取用户列表数据
                        return render(request, "admins/users_manager.html",
                                      {"users_data_list": users_data_list, "user_name": user_name})
                    # ----------------------GET请求可以返回内容的开始---------------------------
                    # ----------------------GET请求可以返回内容的开始---------------------------
            ...
            logger.exception("function users_manager - %s - requestx.COOKIES.get is false - %s" % (ip, e),
                             exc_info=True)
            return redirect("/admins/login/")
        # -------------------
    elif request.method == "POST":
        ...
        try:
            ...
            logger.warning("function users_manager  - %s - updata times %s - %s - %s" % (
                ip, user_id, updata_time, users_alives_time_s))
            # 如果不等于空，就可以进行更新数据库
            if user_id != None and updata_time != None and users_alives_time_s != None:
                ...
                logger.warning(
                    "function users_manager  - %s - user_id == None and updata_time == None and  users_alives_time_s == None" % (
                        ip))
                return HttpResponse("404")
            # ----------------------POST请求可以返回内容的开始---------------------------
            # ----------------------POST请求可以返回内容的开始---------------------------
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if search_admins_account_result != ():
                ...
                logger.warning("function users_disk_manager - %s - search_admins_account_result = %s" % (
                    ip, search_admins_account_result))
                # 获取他的数据库session，判断存活时间是否大于现在时间，如果小于这退出。
                sql = '''select * from admins_Session where admins_account = %s'''
                search_admins_session_result = manager_sqlx.search(sql, [user_name])
                if search_admins_session_result != ():
                    ...
                    logger.warning("function users_disk_manager - %s - search_admins_Session_result = %s" % (
                        ip, search_admins_session_result))
                    # 返回正常页面
                    if int(eval(search_admins_session_result[0][2]).get("alive_time_s")) <= int(time.time()):
                        # 代表管理员账户已过期
                        logger.warning(
                            "function users_disk_manager - %s - search_admins_Session_result[0][3]['alive_time_s'] is false" % (
                                ip))
                        return redirect("/admins/login/")
                    elif int(eval(search_admins_session_result[0][2]).get("alive_time_s")) > int(time.time()):
                        logger.warning(
                            "function users_disk_manager - %s - search_admins_Session_result[0][3]['alive_time_s'] is true" % (
                                ip))
                        # ----------------------GET请求可以返回内容的开始---------------------------
                        # ----------------------GET请求可以返回内容的开始---------------------------
        ...
        try:
            ...
            # 这里应该还需要认证一次是否已经登入
            # 这里应该还需要认证一次是否已经登入
            global users_disk_manager_choose_list
            try:
                uuid4_str = request.COOKIES.get("adminsname_id")
                user_name = str(request.session.get(uuid4_str))
                logger.warning("function users_disk_manager - %s - user_name = %s" % (ip, user_name))
            ...
            if search_content != "fall":
                ...
                search_uses_network_dick_result = users_disk_manager_choose_list
                # print("\n#############>>>：(search_content != "")：",type(users_disk_manager_choose_list),users_disk_manager_choose_list[0])
                if search_uses_network_dick_result == None:
                    logger.warning("function users_disk_manager - %s - search_uses_network_dick_result == ()" % (ip))
                    return HttpResponse("404")
                elif search_uses_network_dick_result != ():
                    logger.warning("function users_disk_manager - %s - search_uses_network_dick_result != ()" % (ip))
                    if search_content == " " or search_content == "0":
                        for index, row in enumerate(search_uses_network_dick_result):
                            ...
                            oo = "A" + str(index) + "B"
                            insert_time = row[7]
                            return_uses_network_dict[oo] = [index, qqclass_name, qqclass_number,
                                                            qqclass_from_user_account,
                                                            user_is_like, download_location, insert_time]
                    else:
                        for index, row in enumerate(search_uses_network_dick_result):
                            qqclass_name = row[1]
                            qqclass_number = row[2]
                            ...
                            oo = "A" + str(index) + "B"
                            insert_time = row[7]
                            if str(search_content) in (
                                    str(qqclass_name) + str(qqclass_number) + str(qqclass_from_user_account)) or str(
                                search_content) == user_is_like:
                                return_uses_network_dict[oo] = [index, qqclass_name, qqclass_number,
                                                                qqclass_from_user_account, user_is_like,
                                                                download_location, insert_time]
            ...
            elif search_content == "fall":
                logger.warning("function users_disk_manager - %s - sarch_content == None" % (ip))
                sql = '''select * from users_network_dick group by qqclass_number'''
                #sql = '''select * from users_network_dick where id in (select max(id) from users_network_dick group by qqclass_number)'''
                return_uses_network_dict = {}
                search_uses_network_dick_result = manager_sqlx.search(sql, [])
                users_disk_manager_choose_list = search_uses_network_dick_result
                # if len(search_uses_network_dick_result) >= 1600:
                #     search_uses_network_dick_result = search_uses_network_dick_result[-1500:-1]
                print("\n#############>>>：(search_content == "")：", type(users_disk_manager_choose_list))
                if search_uses_network_dick_result == ():
                    ...
                    for index, row in enumerate(search_uses_network_dick_result):
                        ...
                        oo = "A" + str(index) + "B"
                        insert_time = row[7]
                        return_uses_network_dict[oo] = [index, qqclass_name, qqclass_number, qqclass_from_user_account,
                                                        user_is_like, download_location, insert_time]
                    sort_dict = dict(sorted(return_uses_network_dict.items(), key=lambda x: x[1][0], reverse=True))
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if search_admins_account_result != ():
                ...
                logger.warning("function activity_manager - %s - search_admins_account_result = %s" % (
                    ip, search_admins_account_result))
                # 获取他的数据库session，判断存活时间是否大于现在时间，如果小于这退出。
                sql = '''select * from admins_Session where admins_account = %s'''
                search_admins_session_result = manager_sqlx.search(sql, [user_name])
                if search_admins_session_result != ():
                    ...
                    logger.warning("function activity_manager - %s - search_admins_Session_result = %s" % (
                        ip, search_admins_session_result))
                    # 返回正常页面
                    if int(eval(search_admins_session_result[0][2]).get("alive_time_s")) <= int(time.time()):
                        # 代表管理员账户已过期
                        logger.warning(
                            "function activity_manager - %s - search_admins_Session_result[0][3]['alive_time_s'] is false" % (
                                ip))
                        return redirect("/admins/login/")
                    elif int(eval(search_admins_session_result[0][2]).get("alive_time_s")) > int(time.time()):
                        logger.warning(
                            "function activity_manager - %s - search_admins_Session_result[0][3]['alive_time_s'] is true" % (
                                ip))
                        # ----------------------Get请求可以返回内容的开始---------------------------
                        # ----------------------Get请求可以返回内容的开始---------------------------
                        ...
                        return render(request, "admins/activity_manager.html",
                                      {"activity_list": search_alls_notice_activity_list_result,
                                       "user_name": user_name})
                        # ----------------------Get请求可以返回内容的开始---------------------------
                        # ----------------------Get请求可以返回内容的开始---------------------------
                elif search_admins_session_result == ():
            ...
            logger.exception("function activity_manager - %s - requestx.COOKIES.get is false - %s" % (ip, e),
                             exc_info=True)
            return redirect("/admins/login/")
        # -------------------
    elif request.method == "POST":
